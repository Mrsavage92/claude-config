...
6. **Last-Mile Security setting MUST match the Mule app's internal listener protocol** — they MUST be the same. LMS controls whether the ingress→pod last hop is HTTPS. If they disagree → **502 Bad Gateway**. External callers always get HTTPS either way (ingress terminates TLS at edge). The mapping:
   - `<http:listener-connection ... protocol="HTTPS" />` (with `tls:context`) → LMS **ON** (CHECKED). Both BDR broadband apps (ITS, Process). Verified 2026-05-15 — forgetting LMS-ON gave 502 on ITS prod.
   - `<http:listener-connection ... />` with no `protocol=` attribute → defaults to HTTP → LMS **OFF** (UNCHECKED). SF System API (`salesforce-system-api/src/main/mule/global-config.xml`). Verified 2026-05-18 — leaving LMS-ON with HTTP listener gave 502. Toggling LMS off resolved it in one rolling restart.
   - To check before deploy: `grep "http:listener-connection" <app>/src/main/mule/global-config.xml` → if you see `protocol="HTTPS"` → LMS ON; if no protocol attribute → LMS OFF. Each app may differ. Don't apply one rule to all of them.
...
8. **Client ID Enforcement is OWASP API02 baseline for prod APIs even with one known consumer** — but BDR's Mule apps DO NOT have `<api-platform-gw:api>` autodiscovery wired in (verified 2026-05-15 via grep of `integrations/*/src/`). Without autodiscovery the apps cannot be registered as "Mule Gateway" type in API Manager. The remaining options are (a) Endpoint with Proxy = deploy a separate Mule Proxy app in front (adds infra + latency) or (b) Endpoint without Proxy = governance-only, no enforcement. Neither is a "click in API Manager today" job. **Doing CIE properly = a v1.0.3 sprint: add autodiscovery to both Mule apps' `global-config.xml` → build new JARs → deploy sandbox → UAT → apply CIE policy in API Manager → generate Connected App per env → update SF External Credential per env → repeat in prod. ~1-2 days work.** For Phase 1 cutover: ship without CIE, calendar v1.0.3 hardening for week 1. Audit MULE-H1 in `docs/DEEP-DIVE-AUDIT-2026-05-13.md` already flagged this. Don't suggest "click in API Manager" without first checking if the Mule source has autodiscovery wired.
