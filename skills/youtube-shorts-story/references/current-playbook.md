...
## THE $0 PRODUCTION MODEL (locked 2026-07-15 - default while pre-traction) - READ THIS FIRST

Founder constraint: "until we have followers and views hitting better numbers I need almost free output." Agent time is free; fal spend is not. **Build every video at $0 unless Adam explicitly approves spend.** Every other section below that names a paid tool (Kling, ElevenLabs, Seedream) is the PAID UPGRADE, gated on explicit per-video approval - not the default.
...
- **Motion = FREE ffmpeg kinetic still-motion** on keyframes. Go ALL-IN, never mix with Kling: the earlier cut was rejected because 6 fluid Kling shots next to 5 static ones reads as broken. **Consistency is what sells it** - a fully-kinetic still-motion cut (hard punch-ins on impacts, slow pushes on calm beats, white-flash hits on zaps) reads as a deliberate style. See shot-design.md's $0 motion vocabulary and its $0-tier worked example.
- **Keyframe stills = reuse the existing character/anatomy library first.** When a genuinely NEW still is needed, generate it FREE via **Pollinations** (model `sana`, no API key, verified 2026-07-16): `https://image.pollinations.ai/prompt/{urlencoded_prompt}?width=1080&height=1920&nologo=true&seed=N`. Hard-capped at 576x1024 - upscale to 1080x1920 with ffmpeg lanczos (`scale=1080:1920:flags=lanczos`) before use.
  **Measured topic constraint (verified 2026-07-16) - this is real, not a footnote:** Pollinations renders common subjects well (fish, cats, people, fur, water) but FAILS on specific biology (goosebumps renders as a rash, skin follicles as blobs, tadpole gills as caterpillars) and on hands. If a video's premise hinges on a close-up biology detail or hand shot Pollinations can't render, either simplify the composition to avoid the failing detail, reuse/adapt an existing keyframe, or ask approval for a paid stills model (Seedream 4, see production-stack.md) instead of shipping a blob. Do not write a shot list that assumes Pollinations can render what it measurably cannot.
- **SFX = source per shot, FREE - never recycle one cue across unrelated videos** (a sleep-episode heartbeat on a fish video reads wrong: the exact 2026-07-15 failure). $0 default = Freesound API (CC0/CC-BY filter, free, scriptable, 2000/day); paid fallback = fal `sound-effects/v2` ONLY (v0 is deprecated = 400 block). Match each cue to its actual shot.
...
- **The first few seconds decide it** - front-load the strongest hook (the algorithm seeds to a small test audience first; weak early engagement caps reach). See retention-playbook.md for what is actually evidenced on hook timing (the hard number is a cliff at 3s, not a proven sub-1s deadline) and compliance-gates.md Gate 8 for the arithmetic gate this skill enforces.
...
- **Cut every 2-3 seconds** for curiosity/story. (NOTE: a CUT is not a PATTERN INTERRUPT. There is exactly ONE deliberate pattern interrupt - a music/camera/SFX reset - at the ~25-35s drift point; see SKILL.md Step 5 + retention-playbook.md. Cut cadence is rhythm; the interrupt is a single reset.) This is the multi-shot EDIT (see shot-design.md).
...
- Use **ElevenLabs v3 Expressive** tier (named leader), genre-matched tone (calm/authoritative for survival/science/history) - PAID TIER, approval required; the $0 default is edge-tts (see the $0 lock above). OpenAI TTS only for plain instructional delivery.
...
## Model + cost (PAID TIER ONLY - see the $0 lock at the top of this file for the actual default)

Kling 1.6 Elements via fal.ai (best value dynamic model, ~$2-4/video) - see production-stack.md + [[reference_ai_video_gen_real_costs]]. This is the paid upgrade path once spend is approved, not the default; the $0 tier uses ffmpeg still-motion (see above).

## PROVEN template + hard-won lessons (video 06 "quicksand", founder-approved 2026-07-14 - "funny, informative, high quality, smooth")

The first fully successful video - built on the PAID tier before the $0 lock existed. Repeat this recipe once spend is approved for a given video; the $0 tier is still the default absent that approval.
...
## Cold start: VOLUME = more independent completion-rate tests (2026-07-16 correction, supersedes the earlier 'follower count dominates' reading)

The arXiv n=400 study found follower count CORRELATES with virality, BUT the platforms' OWN docs (TikTok Newsroom, YouTube Help, OFFICIAL) say followers are NOT a direct ranking input - every video is tested independently on COMPLETION RATE. Reconciliation: more uploads = more independent completion-rate tests, not accumulated authority. Volume is the lever, not follower-count-as-cause. # source: https://arxiv.org/pdf/2111.02452 (ACM CHI PLAY, n=400 labelled TikToks)
A channel with 3 uploads and no audience is STRUCTURALLY disadvantaged regardless of craft. Independently confirmed by our own data: 3 videos, no signal, and a full session was wasted A/B-ing claymation-vs-realistic on n=3 (2026-07-15).
**Rule: do NOT A/B style, character or topic on fewer than ~10 videos / <1000 views. At that volume everything is noise.** Volume precedes any judgement about content.
